...
const handleSearchDevices = async () => {
  ...
  try {
    ...
    message.value = '搜索失败: ' + (e?.message || '未知错误')
...
defineExpose({ loadLastDevice, status, message, info })
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-3 shadow-sm"
      ...
      <div v-if="searchedDevices.length > 0" class="space-y-2">
        ...
        <button @click="handleDeviceConnect"
          ...
          <component :is="status === 'connecting' ? Loader2 : Power" :size="14"
                     :class="{'animate-spin': status === 'connecting'}"/>
          {{ deviceButtonLabel }}
        </button>

        <!-- 设备截图预览 -->
        <div v-if="status === 'connected' && deviceScreenshot" 
